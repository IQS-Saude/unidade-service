...
export class UnidadesController extends BaseController {
  ...
  async criar(@Body() request: CriarUnidadeDataRequest) {
    return this.success(
      this.commandBus.execute(new CriarUnidadeCommand(request.data)),
    );
  }
  ...
  async atualizar(@Body() request: AtualizarUnidadeDataRequest) {
    return this.success(
      this.commandBus.execute(new AtualizarUnidadeCommand(request.data)),
    );
  }
  ...
  async listarTodos(@Query('status', ParseBoolPipe) status: boolean) {
    return this.success(
      this.queryBus.execute(new ListarTodasUnidadesQuery(status)),
    );
  }

  @Get('/unidade/:id')
  async buscarPorId(@Param('id') id: number) {
    return this.success(this.queryBus.execute(new BuscarUnidadePorIdQuery(id)));
  }

  @Post('/unidade/:id/ativar')
  async ativar(@Param('id') id: number) {
    return this.success(this.commandBus.execute(new AtivarUnidadeCommand(id)));
  }

  @Post('/unidade/:id/desativar')
  async desativar(@Param('id') id: number) {
    return this.success(
      this.commandBus.execute(new DesativarUnidadeCommand(id)),
    );
  }

  @Get('/dashboard')
  async dashboard() {
    return this.success(this.queryBus.execute(new DashboardUnidadeQuery()));
  }
}
